...
mkdirs:
	...
	mkdir -p ${OUTDIR}/iso/img
iso:
	cp src/grub.cfg ${OUTDIR}/iso/boot/grub
	cp ${OUTDIR}/bins/ospaint.bin ${OUTDIR}/iso/boot
	cp img/ospaint_splash.bmp ${OUTDIR}/iso/img

	grub-mkrescue -o ${OUTDIR}/ospaint.iso ${OUTDIR}/iso
boot:
	nasm -felf32 src/asm/boot.asm -o ${OUTDIR}/objs/boot.o
kernel:
	gcc -c -I src src/kmain.c -o ${OUTDIR}/objs/kernel.o -std=gnu99 -ffreestanding -Wall -Wextra -m32 -fno-pic -no-pie -g -mgeneral-regs-only -mno-red-zone -fno-stack-protector -Os -w
bin:
